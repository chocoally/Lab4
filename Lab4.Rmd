...
pca_combat <- prcomp(t(combat_edata), center = TRUE, scale. = TRUE)
combat_pc_df <- as.data.frame(pca_combat$x)
combat_pc_df$batch <- as.factor(pheno$batch)
combat_pc_df$hasCancer <- as.factor(pheno$hasCancer)

ggplot(combat_pc_df, aes(x = PC1, y = PC2, color = batch)) +
    geom_point()
ggplot(combat_pc_df, aes(x = PC1, y = PC2, color = hasCancer)) +
    geom_point()
```
